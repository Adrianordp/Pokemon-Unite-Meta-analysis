...
class RelevanceStrategy(Protocol):
    def __call__(self, builds: list[Build], threshold: float, get_builds: Callable[[], list[Build]]) -> list[Build]:
        ...


def relevance_any(builds: list[Build], threshold: float, get_builds: Callable[[], list[Build]]) -> list[Build]:
    return builds

def relevance_moveset_item_true_pr(builds: list[Build], threshold: float, get_builds: Callable[[], list[Build]]) -> list[Build]:
    return [build for build in builds if build.moveset_item_true_pick_rate >= threshold]

def relevance_position_of_popularity(builds: list[Build], threshold: float, get_builds: Callable[[], list[Build]]) -> list[Build]:
    ...
    return [build for build in builds if build.moveset_item_true_pick_rate >= cut_value]
